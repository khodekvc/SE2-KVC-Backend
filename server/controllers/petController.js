...
exports.updatePetProfile = async (req, res) => {
  ...
  try {
    ...
    const isAgeUpdated =
      updatedData.pet_age_year !== undefined ||
      updatedData.pet_age_month !== undefined;
    ...
    if (isBirthdayUpdated) {
      ...
      if (!birthDate.isValid()) {
        return res
          .status(400)
          .json({ error: "❌ Invalid pet_birthday format. Use YYYY-MM-DD." });
      }
    ...
    if (
      ...
      !updateFields.speciesDescription
    ) {
    ...
    if (Object.keys(updatedData).length > 0) {
      ...
      if (result.affectedRows === 0) {
        return res.status(400).json({
          error:
            "❌ No changes made (pet ID may not exist or data is the same).",
        });
      }
    }
    ...
    if (updateFields.speciesDescription) {
      const species = await PetModel.findSpeciesByDescription(
        updateFields.speciesDescription
      );
      if (!species) {
