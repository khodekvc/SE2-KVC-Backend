...
class UserModel {
  ...
  static async getOwnerByUserId(userId) {
    const [rows] = await db.execute("SELECT * FROM owner WHERE user_id = ?", [
      userId,
    ]);
    return rows.length ? rows[0] : null;
  }
  ...
  static async updatePassword(userId, hashedPassword) {
    return db.execute("UPDATE users SET user_password = ? WHERE user_id = ?", [
      hashedPassword,
      userId,
    ]);
  }
  ...
  static async findByEmail(email) {
    const sql =
      "SELECT user_id, user_password, user_role FROM users WHERE user_email = ?";
